...
public class ExpandableTextView extends TextView {
    ...
    private String mEllipsisHint;//省略号提示的文本
    private String mToExpandHint;//折叠状态时展开提示的文本
    private String mToShrinkHint;//展开状态时收起提示的文本
    private String mGapToExpandHint = GAP_TO_EXPAND_HINT;//提示文本与内容之间的间距
    private String mGapToShrinkHint = GAP_TO_SHRINK_HINT;
    private boolean mToggleEnable = TOGGLE_ENABLE;
    private boolean mShowToExpandHint = SHOW_TO_EXPAND_HINT;//是否显示展开提示文本
    private boolean mShowToShrinkHint = SHOW_TO_SHRINK_HINT;//是否显示收起提示文本
    private int mMaxLinesOnShrink = MAX_LINES_ON_SHRINK;//折叠状态时可显示的最大行数
    ...
    private int mCurrState = STATE_SHRINK;//控件当前是收起还是展开状态
    ...
    private void init() {
        ...
        if(TextUtils.isEmpty(mToExpandHint)){
            mToExpandHint = getResources().getString(R.string.expandable_text_view_to_expand_hint);
        }
        if(TextUtils.isEmpty(mToShrinkHint)){
            mToShrinkHint = getResources().getString(R.string.expandable_text_view_to_shrink_hint);
        }
        if(mToggleEnable){
    ...
    public class LinkTouchMovementMethod extends LinkMovementMethod {
        ...
        private TouchableSpan getPressedSpan(TextView textView, Spannable spannable, MotionEvent event) {
            ...
            int line = layout.getLineForVertical(y);//获得点击所在的那一行
