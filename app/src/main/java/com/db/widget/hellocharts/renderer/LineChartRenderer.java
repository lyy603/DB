package com.db.widget.hellocharts.renderer;
...
import com.db.widget.hellocharts.view.Chart;
...
//        canvas.drawCircle(100, 200, 10, pointPaint);
    ...
    private void drawLabelPath(Canvas canvas, Path path, List<PointValue> list, int color) {
        ...
        //曲线布局在屏幕上显示的宽度
        float contentMaxSize = Float.NaN;
        //滑动点距离屏幕左边的长度
        float marginLeft = Float.NaN;
        //当前滑动的距离相当于可滑动距离的百分比
        float percent = Float.NaN;
        //滑动点当前所在坐标距离曲线起点的真实距离
...
//            canvas.drawCircle(100, 200, 10, pointPaint);
        ...
        rawXEnd = computator.computeRawX(list.get(list.size() - 1).getX());

        viewMaxSize = rawXEnd - rawXStart;
        marginLeft = computator.getContentRectMinusAllMargins().left;
